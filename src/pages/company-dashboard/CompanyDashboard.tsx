...
import { MapPin, MessageSquare, FileText, Briefcase, Send, CheckCircle, Bell } from "lucide-react";
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbList,
...
const CompanyDashboard = () => {
  ...
  const stats = [
    { 
      ...
      path: "/company/projects?tab=inProgress" 
    },
    { 
      ...
      path: "/company/projects?tab=waitingForAdminDecision" 
    },
    { 
    ...
    <DashboardLayout userType="company">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="bg-hexa-card border-hexa-border hover:border-hexa-secondary/50 transition-colors cursor-pointer" onClick={() => navigate("/company/notifications")}>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
          ...
          <Card className="bg-hexa-card border-hexa-border hover:border-hexa-secondary/50 transition-colors cursor-pointer" onClick={() => navigate("/company/messages")}>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-hexa-secondary/20 rounded-lg">
                  ...
                  {chatRooms.map((room) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => navigate(`/company/messages?room=${room._id}`)}
                      >
                ...
                {activeProjects.map((project) => {
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => navigate(`/company/projects/${projectId}`)}
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <h3 className="font-medium text-hexa-text-dark">{project.title}</h3>
                          {project.location && (
                            <div className="flex items-center gap-2 text-sm text-hexa-text-light mt-1">
                      ...
                      <div className="flex gap-2 mt-4">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/company/messages?project=${projectId}`);
                          }}
          ...
          <Card className="bg-hexa-card border-hexa-border">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              <div>
                ...
                <CardDescription className="text-hexa-text-light mt-1">
                  {language === "en" ? "Your completed projects" : "مشاريعك المكتملة"}
                </CardDescription>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => navigate("/company/projects?tab=completed")}
                className="border-hexa-border bg-hexa-card text-hexa-text-light hover:bg-hexa-secondary/20 hover:text-hexa-secondary hover:border-hexa-secondary"
              >
                {language === "en" ? "View All" : "عرض الكل"}
              </Button>
              ...
              <div className="space-y-4">
                {completedProjects.map((project) => {
                  ...
                  const location = project.location || 
                    (project.city && project.country ? `${project.city}, ${project.country}` : 
                        ...
                        <div className="pt-3 border-t border-hexa-border">
                          <span className="text-xs text-hexa-text-light">
                            {language === "en" 
                              ...
                              : `مكتمل في ${new Date(project.updatedAt).toLocaleDateString('ar-SA')}`}
                          </span>
                        </div>
                      )}
                    </div>
