  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data && res.data.token) {
        ...
        if (!isCompany) {
          setError(language === 'en' 
            ? 'Access denied. Company credentials required.' 
            : 'تم الرفض. يلزم بيانات اعتماد الشركة.');
          setLoading(false);
          return;
        }

        console.log('Company login success:', res.data);
        ...
        <CardHeader className="text-center">
          <div className="mb-2">
            <p className="text-sm font-semibold text-hexa-secondary uppercase tracking-wider">
              {language === 'en' ? 'Company Login' : 'تسجيل دخول الشركة'}
            </p>
          </div>
          <div className="mx-auto mb-4 w-16 h-16 bg-hexa-secondary/10 flex items-center justify-center hexagon">
            <HexagonIcon size="xl" className="text-hexa-secondary">
              <Building2 className="h-8 w-8 text-hexa-secondary" />
            </HexagonIcon>
          </div>
          ...
          <CardDescription className="text-hexa-text-light">
            {language === 'en' 
              ...
              : 'أدخل بيانات الاعتماد للوصول إلى لوحة تحكم الشركة'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input 
                ...
                placeholder={language === 'en' ? "company@example.com" : "الشركة@example.com"} 
                autoComplete="email"
            ...
            <div className="text-center mt-4">
              ...
              <p className="text-sm text-hexa-text-light">
                {language === 'en' 
                  ...
                  : "ليس لديك حساب؟ "}
                <Link
                  to="/auth/partner?mode=register&type=company"
